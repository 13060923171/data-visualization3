...
df['评论分词'] = df['博文内容'].apply(get_cut_words)
df['发博用户ip'] = df['发博用户ip'].apply(area_process)
df['发博时间'] = df['发博时间'].apply(time_process)
new_df = df.dropna(subset=['评论分词'], axis=0)
new_df.to_csv('博文数据.csv', encoding='utf-8-sig', index=False)
