...
def lda(df,number,k):
    ...
    if not os.path.exists(f"./{number}月"):
        os.mkdir(f"./{number}月")
    ...
    #
    # plt.savefig(f'./{number}月/困惑度和一致性.png')
    #
    # # 将上面获取的数据进行保存
    # df5 = pd.DataFrame()
    # df5['主题数'] = x_data
    # df5['困惑度'] = y_data
    # df5['一致性'] = z_data
    # df5.to_csv(f'./{number}月/困惑度和一致性.csv', encoding='utf-8-sig', index=False)

    # optimal_z = min(y_data)
    # optimal_z_index = y_data.index(optimal_z)
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, f'./{number}月/{number}月.html')
    ...
    df.to_csv(f'./{number}月/{number}月_data.csv', encoding='utf-8-sig', index=False)
    ...
    for j, k, l in zip(topic, list_gailv1, list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv(f'./{number}月/主题词分布表.csv', encoding='utf-8-sig', index=False)

    y_data2 = []
    for y in y_data1:
        number1 = float(y / sum(y_data1))
        y_data2.append(float('{:0.5}'.format(number1)))
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv(f'./{number}月/特征词.csv', encoding='utf-8-sig', index=False)


if __name__ == '__main__':
    list_number = [x for x in range(1, 13)]
    list_k = [9,13,13,8,9,8,8,12,16,14,9,9]
    for n,k in zip(list_number,list_k):
        df = pd.read_csv(f'{n}月.csv')
        lda(df,n,k)