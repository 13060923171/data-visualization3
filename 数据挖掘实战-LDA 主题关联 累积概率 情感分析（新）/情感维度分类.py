...
def lad_probability(name):
    ...
    word_topic = pd.read_csv(f'./{name}/主题词分布表.csv',encoding='gbk')
    ...
    def emotion_type(word):
        try:
            ...
            # 遍历每个词语，并找出它出现在哪个列表
            for word in words:
                found_lists = [f"{list_type[i]}" for i, lst in enumerate(list_content) if word in lst]
                if found_lists:
    ...
    list_dimensions = []
    for i in df['维度'].value_counts().index:
    ...
    negative_values = []
    try:
        for item in list_emotion:
            positive_values.append(item[1]['正面'])
    except:
        positive_values.append(0)
    try:
        for item in list_emotion:
            negative_values.append(item[1]['负面'])
    except:
