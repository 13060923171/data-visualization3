...
def search_by_questionid(huatiid, huatiname):
    for page in range(0, 1000):
        next_page = {
            "page": f'/api/v5.1/topics/{huatiid}/feeds/essence?offset={page * 20}&limit=20',
        }
        ...
        for i in (res["data"]):
            try:
                ...
                saveitem["电影话题id"] = huatiid
                saveitem["回复id"] = target.get("id")
                saveitem["回答者"] = target.get("author", {}).get("name")
                saveitem["回答赞同数"] = target.get("voteup_count")
                saveitem["回答内容摘要"] = target.get("excerpt")
                saveitem["回答内容"] = getele(target.get("content"))
                saveitem["回答评论数"] = target.get("comment_count")
                saveitem["回答创建时间"] = time.strftime('%Y-%m-%d', time.localtime(target.get("updated_time")))
                with open("post.txt", 'a', encoding='utf-8') as f:
                    f.write(json.dumps(saveitem))
                    f.write('\n')
                print(conf["p"],huatiname, page, saveitem["回答内容"][:80] + "...")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    for record in records:
        ...
        search_by_questionid(huatiid=record.get("话题id"), huatiname=record.get("电影名"))
    with open("post.txt",'r',encoding='utf-8') as f:
        lines = [json.loads(i.strip()) for i in f.readlines()]
    df= pandas.DataFrame(lines)
    df.to_excel("帖子.xlsx",index=False)
