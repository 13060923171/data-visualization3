...
# 提取月份并创建一个新的列来存储月份信息
df1['博文发布日期'] = df1['博文发布时间'].dt.to_period('D')
df1['发帖数量'] = 1
...
def picture1():
    ...
    # 设置图表标题
    plt.title('发帖数量与收盘价的关系')

    # 自动调整布局
    plt.tight_layout()
    plt.savefig('发帖数量与收盘价的关系.png')
    # 显示图表
    plt.show()
...
def picture2():
    ...
    # 绘制发帖数量的柱状图
    ax1.bar(data3['日期'], data3['情感得分'], color='red', label='情感热度', alpha=0.8)
    ax1.set_xlabel('日期')
    ax1.set_ylabel('情感热度')
    ax1.tick_params(axis='y')
    # 设置横坐标标签的显示间隔（例如每隔7个日期显示一个标签）
    ...
    plt.rcParams['font.sans-serif'] = ['SimHei']
    # 设置图表标题
    plt.title('情感热度与收盘价的关系')

    # 自动调整布局
    plt.tight_layout()
    plt.savefig('情感热度与收盘价的关系.png')
    # 显示图表
    plt.show()
