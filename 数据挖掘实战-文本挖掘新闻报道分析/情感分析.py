...
#读取数据
data = pd.read_excel('纽约日报.xlsx')
data.drop_duplicates(subset=['链接'],keep='first', inplace=True)
#设置停用词库
stop_words = []
...
data['new_内容'] = data['new_内容'].apply(gettext)
#清洗步骤2
data['new_内容'] = data['new_内容'].apply(preprocess_word)
#清洗步骤3
data['new_内容'] = data['new_内容'].apply(clean_text)
