...
def lda(df,sentiment):
    ...
    if not os.path.exists("./LDA/{}".format(sentiment)):
        os.mkdir("./LDA/{}".format(sentiment))
    ...
    plt.savefig('./LDA/{}/困惑度和一致性.png'.format(sentiment))
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./LDA/{}/困惑度和一致性.csv'.format(sentiment),encoding='utf-8-sig',index=False)
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './LDA/{}/lda.html'.format(sentiment))
    ...
    data = pd.DataFrame()
    data['内容'] = df['new_content']
    data['主题概率'] = list3
    data['主题类型'] = list2

    data.to_csv('./LDA/{}/lda_data.csv'.format(sentiment),encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./LDA/{}/主题词分布表.csv'.format(sentiment), encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./LDA/{}/特征词.csv'.format(sentiment),encoding='utf-8-sig',index=False)
