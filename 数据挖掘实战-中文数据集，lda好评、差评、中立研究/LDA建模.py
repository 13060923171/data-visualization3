...
def lda(name):
    ...
    data.to_csv('./{}/lda_data.csv'.format(name),encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./{}/主题词分布表.csv'.format(name), encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./{}/特征词.csv'.format(name),encoding='utf-8-sig',index=False)
