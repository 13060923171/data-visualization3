...
def loadfile(file_dir, label):
    ...
    for file in file_dir:
        words_list.append(tokenize_only(file))
...
def train_data():
    ...
    df3['评论内容'] = df3['评论内容（仅保留有效内容）']
    df3['评论内容'] = df3['评论内容'].apply(gettext)
    df3['评论内容'] = df3['评论内容'].apply(preprocess_word)
    df3['评论内容'] = df3['评论内容'].apply(clean_text)
    df3 = df3.dropna(how='any',axis=0)
    new_df = df3['情感'].value_counts()
    ...
    for x in x_data:
        data = df3['评论内容（仅保留有效内容）'][df3['情感'] == x]
