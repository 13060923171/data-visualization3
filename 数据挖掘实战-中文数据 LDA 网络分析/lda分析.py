...
def lda(df):
    ...
    if not os.path.exists("./评论文本-lda"):
        os.mkdir("./评论文本-lda")
    ...
    plt.savefig('./评论文本-lda/困惑度和一致性.png')
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./评论文本-lda/困惑度和一致性.csv', encoding='utf-8-sig', index=False)
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, './评论文本-lda/评论文本-lda.html')
    ...
    df.to_csv('./评论文本-lda/评论文本-lda_data.csv', encoding='utf-8-sig', index=False)
    ...
    for j, k, l in zip(topic, list_gailv1, list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./评论文本-lda/主题词分布表.csv', encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./评论文本-lda/特征词.csv', encoding='utf-8-sig', index=False)


if __name__ == '__main__':
    df = pd.read_csv('评论文本数据.csv')
    lda(df)