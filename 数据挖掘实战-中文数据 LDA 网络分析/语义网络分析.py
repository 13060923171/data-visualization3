...
def main1(df,name):
    word = list(set(' '.join(df['人名']).split()))  # 获取所有唯一词汇
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(process_line, line) for line in df['人名']]
    ...
    df = df.sort_values(by='weight', ascending=False).iloc[:30]
