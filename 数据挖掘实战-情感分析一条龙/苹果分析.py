...
def snownlp_fx():
    ...
    df1.to_csv('./苹果-data/苹果-snownlp情感分析.csv', encoding='utf-8-sig', index=False)


def wordclound_fx():
    df = pd.read_csv('./苹果-data/苹果-snownlp情感分析.csv')
    ...
    stylecloud.gen_stylecloud(text=' '.join(text1), max_words=100,
                              ...
                              output_name='./苹果-data/正向词云图.png')
    Image(filename='./苹果-data/正向词云图.png')
    ...
    stylecloud.gen_stylecloud(text=' '.join(text2), max_words=100,
                              ...
                              size=500,
                              # palette='matplotlib.Inferno_9',
                              output_name='./苹果-data/负向词云图.png')
    Image(filename='./苹果-data/负向词云图.png')
    ...
    stylecloud.gen_stylecloud(text=' '.join(text3), max_words=100,
                              ...
                              size=500,
                              # palette='matplotlib.Inferno_9',
                              output_name='./苹果-data/总体词云图.png')
    Image(filename='./苹果-data/总体词云图.png')
    ...
    df1['word'] = x_data
    df1['counts'] = y_data
    df1.to_csv('./苹果-data/高频词.csv', encoding="utf-8-sig")
    ...
    df1['word'] = x_data
    df1['counts'] = y_data
    df1.to_csv('./苹果-data/正向-高频词.csv', encoding="utf-8-sig")
    ...
    df1['word'] = x_data
    df1['counts'] = y_data
    df1.to_csv('./苹果-data/负向-高频词.csv', encoding="utf-8-sig")
...
def lda_tfidf():
    ...
    f = open('./苹果-data/苹果-class-fenci.txt', 'w', encoding='utf-8')
    ...
    # 读取预料 一行预料为一个文档
    for line in open('./苹果-data/苹果-class-fenci.txt', 'r', encoding='utf-8').readlines():
    ...
    df2.to_csv('./苹果-data/苹果-tfidf.csv', encoding='utf-8-sig', index=False)

    fr = open('./苹果-data/苹果-class-fenci.txt', 'r', encoding='utf-8')
    train = []
    ...
    plt.xlabel('主题数')
    plt.ylabel('平均余弦相似度')
    plt.savefig('./苹果-data/LDA评论主题数寻优.png')
    plt.show()
    ...
    pyLDAvis.save_html(data, './苹果-data/苹果-lda.html')
