...
def lda(df):
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv(f'./lda/困惑度和一致性.csv', encoding='utf-8-sig', index=False)
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, f'./lda/评论文本-lda.html')
    ...
    df.to_csv(f'./lda/lda_data.csv', encoding='utf-8-sig', index=False)
    ...
    for j, k, l in zip(topic, list_gailv1, list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv(f'./lda/主题词分布表.csv', encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv(f'./lda/特征词.csv', encoding='utf-8-sig', index=False)
