...
def main():
    ...
    df['物流信息'] = df['物流信息'].apply(emjio_tihuan)
    df = df.dropna(subset=['物流信息'], axis=0)

    def fenci():
        f = open('./网络语义/fenci.txt', 'w', encoding='utf-8-sig')
        for line in df['物流信息']:
            line = line.strip('\n')
            # 停用词过滤
    ...
    word = [] #记录关键词
    f = open("./网络语义/fenci.txt", encoding='utf-8')
    line = f.readline()
    while line:
        #print line
        line = line.replace("\n", "") #过滤换行
    ...
    words = codecs.open("./网络语义/word_node_1.txt", "w", "utf-8")
    i = 0
    while i < len(word):
        student1 = word[i]
        j = i + 1
    ...
    """ 第四步:图形生成 """
    with open('./网络语义/word_node_1.txt','r',encoding='utf-8')as f:
        content = f.readlines()
    list_word1 = []
    list_word2 = []
    list_weight = []
