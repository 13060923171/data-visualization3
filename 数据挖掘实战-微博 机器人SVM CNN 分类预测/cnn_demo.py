...
df1 = pd.read_excel('./data/data.xlsx',sheet_name='正常数据')
df2 = pd.read_excel('./data/data.xlsx',sheet_name='简介为空或关粉比大于3')
...
model = tf.keras.Sequential([
    tf.keras.layers.Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=X_train.shape[1:]),  # 修改输入形状
...
plt.savefig('./data/CNN_ROC曲线.png')
# 绘制PR曲线
plt.figure()
...
plt.savefig('./data/CNN_PR曲线.png')

#创建分类报告
clf_report = classification_report(y_test, y_pred_classes)

# 将分类报告保存为 .txt 文件
with open('./data/classification_report_cnn.txt', 'w') as f:
    f.write(clf_report)
