...
def lda(df):
    ...
    if not os.path.exists("./LDA-post/"):
        os.mkdir("./LDA-post/")
    ...
    plt.savefig('./LDA-post/困惑度和一致性.png')
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./LDA-post/困惑度和一致性.csv',encoding='utf-8-sig',index=False)
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './LDA-post/lda.html')
    ...
    df.to_csv('./LDA-post/lda_data.csv',encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./LDA-post/主题词分布表.csv', encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./LDA-post/特征词.csv',encoding='utf-8-sig',index=False)
...
if __name__ == '__main__':
    # df1 = pd.read_excel('en_comment.xlsx')
    # df2 = pd.read_csv('zh_comment_trans.csv')
    # df2 = df2.drop(['分词'],axis=1)
    # df2 = df2.rename(columns={'分词-英文': '分词'})
    # df3 = pd.read_csv('zh_comment_trans1.csv')
    # df3 = df3.drop(['分词'], axis=1)
    # df3 = df3.rename(columns={'分词-英文': '分词'})
    # df = pd.concat([df1,df2,df3],axis=0)
    # df = df.dropna(subset=['分词'],axis=0)
    # lda(df)

    df1 = pd.read_excel('en_post.xlsx')
    df2 = pd.read_csv('zh_post_trans.csv')
    df2 = df2.drop(['分词'], axis=1)
    df2 = df2.rename(columns={'分词-英文': '分词'})
