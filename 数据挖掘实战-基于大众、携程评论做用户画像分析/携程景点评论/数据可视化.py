...
def emotion_type():
    df = pd.read_excel('新—携程-大昭寺.xlsx')
    ...
    new_df.to_csv('情感分类_博文_大昭寺.csv',encoding='utf-8-sig')
    plt.pie(y_data, labels=x_data, startangle=0, autopct='%1.2f%%')
    plt.title('情感分类')
    # 添加图例
    plt.legend(x_data, loc='lower right')
    plt.tight_layout()
    plt.savefig('情感分类_博文_大昭寺.png')
...
def bar_1():
    ...
    def demo(x):
        ...
        x1 = x1[0]
        return x1
    df['publishTypeTag'] = df['publishTypeTag'].apply(demo)
    df['publishTypeTag'] = pd.to_datetime(df['publishTypeTag'])
    df.index = df['publishTypeTag']
...
def word_1():
    ...
    data.to_csv('大昭寺_高频词Top100.csv',encoding='utf-8-sig',index=False)
    # 然后传入词云图中，筛选最多的100个词
    stylecloud.gen_stylecloud(text=' '.join(list_text), max_words=100,
                              ...
                              # 输出图片的名称和位置
                              output_name='大昭寺_词云图.png')
    # 开始生成图片
    Image(filename='大昭寺_词云图.png')
