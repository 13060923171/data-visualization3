...
df1 = pd.read_excel('./百词斩-lda/百词斩-lda_data(二级维度-技术设计).xlsx')
df1['二级-主题类型'] = df1['二级-主题类型'].replace(0,'易于使用').replace(1,'屏幕设计').replace(2,'目标导向').replace(3,'交互性')
df2 = pd.read_excel('./百词斩-lda/百词斩-lda_data(二级维度-艺术审美).xlsx')
df2['二级-主题类型'] = df2['二级-主题类型'].replace(0,'学习者控制').replace(1,'兴趣').replace(2,'实用').replace(3,'个人喜好')

df6 = pd.concat([df1,df2],axis=0)

df3 = pd.read_excel('./百词斩-lda/特征维度(二级维度-技术设计).xlsx')
df3['所属主题'] = df3['所属主题'].replace('Topic0','易于使用').replace('Topic1','屏幕设计').replace('Topic2','目标导向').replace('Topic3','交互性')
df4 = pd.read_excel('./百词斩-lda/特征维度(二级维度-艺术审美).xlsx')
df4['所属主题'] = df4['所属主题'].replace('Topic0','学习者控制').replace('Topic1','兴趣').replace('Topic2','实用').replace('Topic3','个人喜好')

df5 = pd.read_excel('./百词斩-lda/特征维度(一级维度).xlsx')
