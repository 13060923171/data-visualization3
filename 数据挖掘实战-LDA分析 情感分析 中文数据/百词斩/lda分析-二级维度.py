...
def lda(df,name):
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, f'./百词斩-lda/百词斩-lda(二级维度-{name}).html')
    ...
    df.to_excel(f'./百词斩-lda/百词斩-lda_data(二级维度-{name}).xlsx', index=False)
    ...
    df1['关注度内部占比'] = y_data2
    df1.to_excel(f'./百词斩-lda/特征维度(二级维度-{name}).xlsx', index=False)


if __name__ == '__main__':
    df = pd.read_excel('./百词斩-lda/百词斩-lda_data(一级维度).xlsx')
    df['一级-主题类型'] = df['一级-主题类型'].replace(0,'技术设计').replace(1,'艺术审美')
    list_number = ['技术设计','艺术审美']
