...
def lda(df):
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, './不背单词-lda/不背单词-lda(一级维度).html')
    ...
    df.to_excel('./不背单词-lda/不背单词-lda_data(一级维度).xlsx', index=False)

    data = df
    # 获取对应主题出现的频次
    new_data = data['一级-主题类型'].value_counts()
    ...
    df1['内部占比'] = y_data2
    df1.to_excel('./不背单词-lda/特征维度(一级维度).xlsx', index=False)


if __name__ == '__main__':
    df = pd.read_excel('新_不背单词_sentiment.xlsx')
    lda(df)