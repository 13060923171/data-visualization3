...
df1 = pd.read_excel('./不背单词-lda/不背单词-lda_data(二级维度-技术设计).xlsx')
df1['二级-主题类型'] = df1['二级-主题类型'].replace(0,'屏幕设计').replace(1,'交互性').replace(2,'目标导向').replace(3,'信息演示')
df2 = pd.read_excel('./不背单词-lda/不背单词-lda_data(二级维度-面向教育).xlsx')
df2['二级-主题类型'] = df2['二级-主题类型'].replace(0,'连接未来学习').replace(1,'学习材料的水平').replace(2,'内容的严格性').replace(3,'21世纪技能')

df6 = pd.concat([df1,df2],axis=0)

df3 = pd.read_excel('./不背单词-lda/特征维度(二级维度-技术设计).xlsx')
df3['所属主题'] = df3['所属主题'].replace('Topic0','屏幕设计').replace('Topic1','交互性').replace('Topic2','目标导向').replace('Topic3','信息演示')
df4 = pd.read_excel('./不背单词-lda/特征维度(二级维度-面向教育).xlsx')
df4['所属主题'] = df4['所属主题'].replace('Topic0','连接未来学习').replace('Topic1','学习材料的水平').replace('Topic2','内容的严格性').replace('Topic3','21世纪技能')

df5 = pd.read_excel('./不背单词-lda/特征维度(一级维度).xlsx')
df5['所属主题'] = df5['所属主题'].replace('Topic0','技术设计').replace('Topic1','面向教育')
