...
df1 = pd.read_excel('./墨墨-lda/墨墨-lda_data(二级维度-面向教育).xlsx')
df1['二级-主题类型'] = df1['二级-主题类型'].replace(0,'连接未来学习').replace(1,'学习材料的水平').replace(2,'教师的反馈').replace(3,'个人差异的包容')
df2 = pd.read_excel('./墨墨-lda/墨墨-lda_data(二级维度-技术设计).xlsx')
df2['二级-主题类型'] = df2['二级-主题类型'].replace(0,'信息演示').replace(1,'屏幕设计').replace(2,'媒体整合').replace(3,'易于使用')

df6 = pd.concat([df1,df2],axis=0)

df3 = pd.read_excel('./墨墨-lda/特征维度(二级维度-面向教育).xlsx')
df3['所属主题'] = df3['所属主题'].replace('Topic0','连接未来学习').replace('Topic1','学习材料的水平').replace('Topic2','教师的反馈').replace('Topic3','个人差异的包容')
df4 = pd.read_excel('./墨墨-lda/特征维度(二级维度-技术设计).xlsx')
df4['所属主题'] = df4['所属主题'].replace('Topic0','信息演示').replace('Topic1','屏幕设计').replace('Topic2','媒体整合').replace('Topic3','易于使用')

df5 = pd.read_excel('./墨墨-lda/特征维度(一级维度).xlsx')
df5['所属主题'] = df5['所属主题'].replace('Topic0','面向教育').replace('Topic1','技术设计')
list_name = ['面向教育','技术设计']
