...
def lda(df,name):
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, f'./扇贝-lda/扇贝-lda(二级维度-{name}).html')
    ...
    df.to_excel(f'./扇贝-lda/扇贝-lda_data(二级维度-{name}).xlsx', index=False)
    ...
    df1['关注度内部占比'] = y_data2
    df1.to_excel(f'./扇贝-lda/特征维度(二级维度-{name}).xlsx', index=False)


if __name__ == '__main__':
    df = pd.read_excel('./扇贝-lda/扇贝-lda_data(一级维度).xlsx')
