...
def lda(df):
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, './扇贝-lda/扇贝-lda(一级维度).html')
    ...
    df.to_excel('./扇贝-lda/扇贝-lda_data(一级维度).xlsx', index=False)
    ...
    df1['内部占比'] = y_data2
    df1.to_excel('./扇贝-lda/特征维度(一级维度).xlsx', index=False)


if __name__ == '__main__':
    df = pd.read_excel('新_扇贝_sentiment.xlsx')
    lda(df)