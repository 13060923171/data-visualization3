...
def kmeans(df,name):
    ...
    if not os.path.exists("./聚类/聚类-{}".format(name)):
        os.mkdir("./聚类/聚类-{}".format(name))
    ...
    df['二级-聚类结果'] = pre
    df.to_csv('./聚类/聚类-{}/聚类结果.csv'.format(name), encoding="utf-8-sig",index=False)
    ...
    plt.title("聚类图")
    plt.savefig('./聚类/聚类-{}/聚类图.jpg'.format(name))
    plt.show()
...
if __name__ == '__main__':
    data1 = pd.read_csv('./聚类/聚类结果.csv')
    x = 2
    data2 = data1[data1['聚类结果'] == x]
    kmeans(data2,x)
