...
new_df = df[df['emotion_type'] == '负面']
new_df = new_df.dropna(subset=['分词'], axis=0)
if not os.path.exists("./负面"):
    os.mkdir("./负面")
...
def lda():
    # 读取数据
    fr = open('./负面/fenci.txt', 'r', encoding='utf-8-sig')
    train = []
    ...
    # df5['困惑度'] = y_data
    # df5['一致性'] = z_data
    # df5.to_csv('./负面/困惑度和一致性.csv', encoding='utf-8-sig', index=False)
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, './负面/lda.html')
    ...
    for j, k, l in zip(topic, list_gailv1, list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./负面/主题词分布表.csv', encoding='utf-8-sig', index=False)
