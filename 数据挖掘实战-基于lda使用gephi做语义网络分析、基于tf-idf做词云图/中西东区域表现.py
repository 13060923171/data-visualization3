...
def tf_idf(df,area,name):
    ...
    # df2.to_csv('./需求三/{}_{}_tfidf.csv'.format(area,name),encoding='utf-8-sig',index=False)
    ...
    #         .render("./需求三/{}_{}_tf-idf.html".format(area,name))
    # )
    ...
    # 存储图像
    wc.to_file("./需求三/{}_{}_tf-idf.png".format(area,name))


#LDA建模
def lda(df,area,name):
    train = []
    stop_words = ['肖战', '北京', '真的', '肖明', '河南', '链接', '有限公司', '哈哈哈', '许凯', '一种', '两个', '只能', '秦施', '杨幂', '工作室', '二八',
                  '视频', '小说', '微博', '孩子', '室友', '姐姐', '刘耀文']
    for line in df['分词']:
    ...
    #
    # plt.savefig('./需求三/{}_{}_困惑度和一致性.png'.format(area,name))
    # plt.show()
    # #将上面获取的数据进行保存
    # df5 = pd.DataFrame()
    # df5['主题数'] = x_data
    # df5['困惑度'] = y_data
    # df5['一致性'] = z_data
    # df5.to_csv('./需求三/{}_{}_困惑度和一致性.csv'.format(area,name),encoding='utf-8-sig',index=False)
    #
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './需求三/{}_{}_lda.html'.format(area,name))
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./需求三/{}_{}_主题词分布表.csv'.format(area,name), encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./需求三/{}_{}_特征词.csv'.format(area,name),encoding='utf-8-sig',index=False)
