...
def lda(df,name,emotion):
    ...
    plt.savefig('./{}/{}-困惑度和一致性.png'.format(name,emotion))
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./{}/{}-困惑度和一致性.csv'.format(name,emotion),encoding='utf-8-sig',index=False)
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './{}/{}-lda.html'.format(name,emotion))
    ...
    df.to_csv('./{}/{}-lda_data.csv'.format(name,emotion),encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./{}/{}-主题词分布表.csv'.format(name,emotion), encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./{}/{}-特征词.csv'.format(name,emotion),encoding='utf-8-sig',index=False)
