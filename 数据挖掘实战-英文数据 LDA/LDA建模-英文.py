...
def lda(df):
    ...
    plt.savefig('./LDA-整体/困惑度和一致性.png')
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./LDA-整体/困惑度和一致性.csv',encoding='utf-8-sig',index=False)
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './LDA-整体/lda.html')
    ...
    df.to_csv('./LDA-整体/lda_data.csv',encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./LDA-整体/主题词分布表.csv', encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./LDA-整体/特征词.csv',encoding='utf-8-sig',index=False)


if __name__ == '__main__':
    df1 = pd.read_excel('2019-采集结果.xlsx')
    df2 = pd.read_excel('2020-采集结果.xlsx')
    df3 = pd.read_excel('2021-采集结果.xlsx')
    df4 = pd.read_excel('2022-采集结果.xlsx')
    df5 = pd.read_excel('2023-采集结果.xlsx')
    df6 = pd.read_excel('2024-采集结果.xlsx')
