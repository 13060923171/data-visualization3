...
def main():
    # word = [] #记录关键词
    # df = pd.read_csv('./data/新_第二组.csv')
    # df = df.sort_values(by='点赞', ascending=False)
    # df = df.iloc[:1000]
    # df['new_推文'] = df['new_推文'].drop_duplicates(keep='first')
    # df = df.dropna(subset=["new_推文"])
    # for d in df['new_推文']:
    #     d = str(d).split(' ')
    #     for i in d:
    #         word.append(i)
    ...
    #         i = i + 1
    #
    # words = codecs.open("./data/ico-word_node.txt", "w", "utf-8")
    # i = 0
    # while i < len(word):
    #     student1 = word[i]
    #     j = i + 1
    ...
    """ 第四步:图形生成 """
    with open("./data/ico-word_node.txt",'r',encoding='utf-8')as f:
        content = f.readlines()
    list_word1 = []
    list_word2 = []
    list_weight = []
    ...
    for w1,w2 in tqdm(zip(new_df['word1'],new_df['word2'])):
        if w1 != "" and len(w1) >= 3 and w2 != "" and len(w2) >=3:
            A.append(w1)
            B.append(w2)
    ...
    nx.draw_networkx_edges(G,pos,width=0.8,alpha=0.8,edge_color='g')
    nx.draw_networkx_labels(G, pos, font_family='sans-serif', alpha=1,font_size='16')
    plt.title("ico-网络语义")
    plt.savefig('./data/ico-网络语义.png')
    plt.show()
