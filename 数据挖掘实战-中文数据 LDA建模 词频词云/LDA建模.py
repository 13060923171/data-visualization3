...
def lda(df,name):
    ...
    with open("stopwords-add.txt", 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            stop_word.append(line.strip())
    with open("stopwords-new.txt", 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            stop_word.append(line.strip())
    ...
    if not os.path.exists("./LDA/{}".format(name)):
        os.mkdir("./LDA/{}".format(name))
    ...
    #
    # plt.savefig('./LDA/{}/困惑度和一致性.png'.format(name))
    #
    # #将上面获取的数据进行保存
    # df5 = pd.DataFrame()
    # df5['主题数'] = x_data
    # df5['困惑度'] = y_data
    # df5['一致性'] = z_data
    # df5.to_csv('./LDA/{}/困惑度和一致性.csv'.format(name),encoding='utf-8-sig',index=False)
    #
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './LDA/{}/lda.html'.format(name))
    ...
    data.to_csv('./LDA/{}/lda_data.csv'.format(name),encoding='utf-8-sig',index=False)
    ...
    for j,k,l in zip(topic,list_gailv1,list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./LDA/{}/主题词分布表.csv'.format(name), encoding='utf-8-sig', index=False)
    ...
    # df1['特征词'] = quanzhong
    # df1['主题强度'] = y_data2
    # df1.to_csv('./LDA/{}/特征词.csv'.format(name),encoding='utf-8-sig',index=False)


if __name__ == '__main__':
    df = pd.read_excel('new_data.xlsx')
    # lda(df)
    # new_df = df['所属领域'].value_counts()
