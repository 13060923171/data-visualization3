...
def lda(df):
    ...
    plt.savefig('./小学-lda/困惑度和一致性.png')
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./小学-lda/困惑度和一致性.csv', encoding='utf-8-sig', index=False)
    ...
    # 把数据进行可视化处理
    pyLDAvis.save_html(data1, './小学-lda/小学-lda.html')
    ...
    df.to_csv('./小学-lda/小学-lda_data.csv', encoding='utf-8-sig', index=False)
    ...
    for j, k, l in zip(topic, list_gailv1, list_word):
        df2['{}-主题词'.format(j)] = l
        df2['{}-权重'.format(j)] = k
    df2.to_csv('./小学-lda/主题词分布表.csv', encoding='utf-8-sig', index=False)
    ...
    df1['特征词'] = quanzhong
    df1['主题强度'] = y_data2
    df1.to_csv('./小学-lda/特征词.csv', encoding='utf-8-sig', index=False)
    ...
    # 计算主题间的余弦相似度
    topic_similarity = cosine_similarity(lda.get_topics())
    topic_similarity = topic_similarity.round(3)
    ...
    # 自定义布局
    fig.update_layout(
        title="LDA Topic Similarity Heatmap",
        width=800,
        height=600
    )
