    ...
    def parse(self, data, page, keyword):
        ...
        for it in tweets[:-2]:
            ...
            try:
                ...
                dic = {}
                dic['查找关键字'] = keyword.split("-filter")[0].strip()
                ...
                dic['账号id'] = core_handle['user_results']['result']['legacy']['screen_name']
                a = core_handle['user_results']['result']['legacy']['screen_name']
                dic['个人主页'] = f'https://twitter.com/{a}'
                dic['发表内容'] = legacy_handle['full_text'].split("https://t.co/")[0]
                dic['博文id'] = legacy_handle['id_str']
                dic['回复数量'] = legacy_handle["reply_count"]
                dic['转发数量'] = legacy_handle["retweet_count"]
                dic['点赞数'] = legacy_handle["favorite_count"]
                dic['引用数量'] = legacy_handle["quote_count"]
                dic["对某人的回复"] = legacy_handle.get("in_reply_to_screen_name", "")  #
                dic["单词长度"] = len(dic["发表内容"].split())
                try:
                ...
                dic['推文链接'] = f'https://twitter.com/user/status/{idd}'
                dic['作者粉丝数量'] = \
                    core_handle['user_results']['result']['legacy'][
                        "followers_count"]
                dic['作者关注数量'] = \
                    core_handle['user_results']['result']['legacy'][
                        "friends_count"]
                dic['作者地理位置'] = \
                    core_handle['user_results']['result']['legacy'][
                        "location"]
                print(keyword, page, dic["发表日期"], dic)
                self.count += 1
                records_all.append(dic)
        ...
        for page in range(1, self.max_page):
            ...
            try:
                params = {
                    ...
                    "variables": r'{"rawQuery":"' + str(keyword) + '","cursor":"' + cursor[
                        "p"] + '","count":20,"querySource":"typed_query","product":"Top"}'
                }
                blogInfo = self.getblog(params)

                tweets = self.parse(blogInfo, page, keyword)
        ...
        while True:
            time_str.append(
                (time.strftime('%Y-%m-%d', time.localtime(start_samp)),
                 time.strftime('%Y-%m-%d', time.localtime(start_samp + 24 * 60 * 60*30)))
            )
            start_samp += 24 * 60 * 60 * 30
            if start_samp > time.mktime(time.strptime(endtime, '%Y-%m-%d')):
                break
        return time_str

    def getxlsx(self):
        df = pandas.DataFrame(records_all)
        df.to_excel(f'博文表.xlsx', index=False)
...
if __name__ == "__main__":
    ...
    records_all = []
    for isuer in ['SanxingduiC','SichuanDaily']:
        tt = Gtwitter(max_page=10000)
        dates = tt.getdaterange('2010-01-01', '2024-03-01')
        for idate1,idate2 in dates[::-1]:
            print(idate1,idate2)
            tt.blockdata(keyword=f'(from:{isuer}) -filter:replies until:{idate2} since:{idate1}')
    tt.getxlsx()
