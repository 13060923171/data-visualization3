...
def main():
    word = [] #记录关键词
    f = open("./data/岗位描述-class-fenci.txt", encoding='utf-8-sig')
    line = f.readline()
    while line:
        #print line
        line = line.replace("\n", "") #过滤换行
    ...
    words = codecs.open("./data/岗位描述-word_node_1.txt", "w", "utf-8")
    i = 0
    while i < len(word):
        student1 = word[i]
        j = i + 1
    ...
    """ 第四步:图形生成 """
    with open('./data/岗位描述-word_node_1.txt','r',encoding='utf-8')as f:
        content = f.readlines()
    list_word1 = []
    list_word2 = []
    list_weight = []
    ...
    plt.title("岗位描述-网络语义")
    plt.savefig('./data/岗位描述-网络语义.png')
    plt.show()
