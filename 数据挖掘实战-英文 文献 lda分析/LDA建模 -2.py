...
def lda(df,name1):
    ...
    if not os.path.exists("./年份"):
        os.mkdir("./年份")
    plt.savefig('./年份/{}_{}_困惑度和一致性.png'.format(name1,t))
    ...
    df5['困惑度'] = y_data
    df5['一致性'] = z_data
    df5.to_csv('./年份/{}_{}_困惑度和一致性.csv'.format(name1,t),encoding='utf-8-sig',index=False)
    ...
    #把数据进行可视化处理
    pyLDAvis.save_html(data1, './年份/{}_{}_lda.html'.format(name1,t))
    ...
    # data['主题类型'] = list2
    #
    # data.to_csv('./年份/lda_data.csv',encoding='utf-8-sig',index=False)
    ...
    #     df2['{}-主题词'.format(j)] = l
    #     df2['{}-权重'.format(j)] = k
    # df2.to_csv('./年份/主题词分布表.csv', encoding='utf-8-sig', index=False)
    #
    # y_data2 = []
    # for y in y_data1:
    ...
    # df1['特征词'] = quanzhong
    # df1['主题强度'] = y_data2
    # df1.to_csv('./年份/特征词.csv',encoding='utf-8-sig',index=False)
